        ...
        <div class="fi-ta-filters-form fi-fo-component-ctn grid gap-y-4 px-4 py-4 sm:px-6">
            <div class="fi-in-affixes flex items-center gap-x-3">
                <div class="fi-in-affix flex items-center gap-x-3 text-sm leading-6 text-gray-950 dark:text-white">
                    <svg class="fi-in-affix-icon h-5 w-5 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <div class="text-sm">
                        <div class="flex items-center gap-6 mb-2">
                            <div class="flex items-center gap-2">
                                <div class="w-4 h-3 bg-warning-100 dark:bg-warning-900 border-l-4 border-warning-400 rounded-sm"></div>
                                <span>Draft rows (auto-save when complete)</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <div class="w-4 h-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-sm"></div>
                        ...
                        <p><strong>Required fields:</strong> Project, Type, Amount, Date, Status</p>
                        <p><strong>Navigation:</strong> Use arrow keys to move between cells • Click any cell to edit • Changes save automatically</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="fi-ta-content relative divide-y divide-gray-200 overflow-x-auto dark:divide-white/10 dark:border-t-white/10" style="min-height: 400px;">
        <table class="fi-ta-table w-full table-auto divide-y divide-gray-200 text-start dark:divide-white/5" id="transaction-table" style="min-width: 1400px;">
            ...
            <tbody>
                ...
                @endphp
                <tr class="fi-ta-row [@media(hover:hover)]:transition [@media(hover:hover)]:duration-75 hover:bg-gray-50 dark:hover:bg-white/5 {{ count($validationErrors) > 0 ? 'bg-warning-50 dark:bg-warning-400/10' : 'bg-warning-50 dark:bg-warning-400/10' }}">
                    <td class="fi-ta-cell p-0 first-of-type:ps-1 last-of-type:pe-1 sm:first-of-type:ps-3 sm:last-of-type:pe-3 {{ in_array('project_key', $validationErrors) ? 'ring-2 ring-danger-600 dark:ring-danger-500' : '' }}">
                        <select wire:change="updateDraftRow('{{ $rowId }}', 'project_key', $event.target.value)"
                            ...
                            @foreach($projects as $key => $label)
                                <option value="{{ $key }}" {{ $row['project_key'] == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    </td>
                    <td class="fi-ta-cell p-0 first-of-type:ps-1 last-of-type:pe-1 sm:first-of-type:ps-3 sm:last-of-type:pe-3 {{ in_array('financial_type', $validationErrors) ? 'ring-2 ring-danger-600 dark:ring-danger-500' : '' }}">
                        <select wire:change="updateDraftRow('{{ $rowId }}', 'financial_type', $event.target.value)"
                                data-row="{{ $rowId }}" data-col="1"
                                class="fi-select-input block w-full border-none bg-transparent py-1.5 pe-8 ps-3 text-base text-gray-950 transition duration-75 placeholder:text-gray-400 focus:ring-0 disabled:text-gray-500 disabled:[-webkit-text-fill-color:theme(colors.gray.500)] disabled:placeholder:[-webkit-text-fill-color:theme(colors.gray.400)] dark:text-white dark:placeholder:text-gray-500 dark:disabled:text-gray-400 dark:disabled:[-webkit-text-fill-color:theme(colors.gray.400)] dark:disabled:placeholder:[-webkit-text-fill-color:theme(colors.gray.500)] sm:text-sm sm:leading-6">
                            <option value="">Type...</option>
                            @foreach($financialTypes as $key => $label)
                                <option value="{{ $key }}" {{ $row['financial_type'] == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <td class="fi-ta-cell p-0 first-of-type:ps-1 last-of-type:pe-1 sm:first-of-type:ps-3 sm:last-of-type:pe-3">
                        <select wire:change="updateDraftRow('{{ $rowId }}', 'serving', $event.target.value)"
                                data-row="{{ $rowId }}" data-col="2"
                                class="fi-select-input block w-full border-none bg-transparent py-1.5 pe-8 ps-3 text-base text-gray-950 transition duration-75 placeholder:text-gray-400 focus:ring-0 disabled:text-gray-500 disabled:[-webkit-text-fill-color:theme(colors.gray.500)] disabled:placeholder:[-webkit-text-fill-color:theme(colors.gray.400)] dark:text-white dark:placeholder:text-gray-500 dark:disabled:text-gray-400 dark:disabled:[-webkit-text-fill-color:theme(colors.gray.400)] dark:disabled:placeholder:[-webkit-text-fill-color:theme(colors.gray.500)] sm:text-sm sm:leading-6">
                            <option value="">Serving...</option>
                            @foreach($servingTypes as $key => $label)
                                <option value="{{ $key }}" {{ $row['serving'] == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    </td>
                    <td class="fi-ta-cell p-0 first-of-type:ps-1 last-of-type:pe-1 sm:first-of-type:ps-3 sm:last-of-type:pe-3 {{ in_array('amount', $validationErrors) ? 'ring-2 ring-danger-600 dark:ring-danger-500' : '' }}">
                    ...
                    <td class="border border-gray-300 dark:border-gray-600 p-0 bg-yellow-50 dark:bg-yellow-900/30">
                        <select wire:change="updateDraftRow('{{ $rowId }}', 'method', $event.target.value)"
                            ...
                            @foreach($transactionMethods as $key => $label)
                                <option value="{{ $key }}" {{ $row['method'] == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <td class="border p-0 bg-yellow-50 dark:bg-yellow-900/30 {{ in_array('status', $validationErrors) ? 'border-red-500 border-2' : 'border-gray-300 dark:border-gray-600' }}">
                        <select wire:change="updateDraftRow('{{ $rowId }}', 'status', $event.target.value)"
                    ...
                    <td class="border border-gray-300 dark:border-gray-600 p-2 bg-yellow-50 dark:bg-yellow-900/30 text-center">
                ...
                <tr class="hover:bg-blue-50 dark:hover:bg-blue-900/30">
                    <td class="border border-gray-300 dark:border-gray-600 p-0 bg-white dark:bg-gray-900">
                        <select wire:change="updateExistingRow({{ $transaction['id'] }}, 'project_key', $event.target.value)"
                                data-row="existing-{{ $transaction['id'] }}" data-col="0"
                                class="w-full h-full border-0 bg-transparent dark:text-white text-sm p-3 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:bg-white dark:focus:bg-gray-800 transition-colors duration-200">
                            @foreach($projects as $key => $label)
                                <option value="{{ $key }}" {{ $transaction['project_key'] == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <td class="border border-gray-300 dark:border-gray-600 p-0 bg-white dark:bg-gray-900">
                        <select wire:change="updateExistingRow({{ $transaction['id'] }}, 'financial_type', $event.target.value)"
                                data-row="existing-{{ $transaction['id'] }}" data-col="1"
                                class="w-full h-full border-0 bg-transparent dark:text-white text-sm p-3 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:bg-white dark:focus:bg-gray-800 transition-colors duration-200">
                            @foreach($financialTypes as $key => $label)
                                <option value="{{ $key }}" {{ $transaction['financial_type'] == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <td class="border border-gray-300 dark:border-gray-600 p-0 bg-white dark:bg-gray-900">
                        <select wire:change="updateExistingRow({{ $transaction['id'] }}, 'serving', $event.target.value)"
                                data-row="existing-{{ $transaction['id'] }}" data-col="2"
                                class="w-full h-full border-0 bg-transparent dark:text-white text-sm p-3 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:bg-white dark:focus:bg-gray-800 transition-colors duration-200">
                            <option value="">Serving...</option>
                            @foreach($servingTypes as $key => $label)
                                <option value="{{ $key }}" {{ $transaction['serving'] == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <td class="border border-gray-300 dark:border-gray-600 p-0 bg-white dark:bg-gray-900">
                        <select wire:change="updateExistingRow({{ $transaction['id'] }}, 'method', $event.target.value)"
                                data-row="existing-{{ $transaction['id'] }}" data-col="4"
                                class="w-full h-full border-0 bg-transparent dark:text-white text-sm p-3 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:bg-white dark:focus:bg-gray-800 transition-colors duration-200">
                            <option value="">Method...</option>
                            @foreach($transactionMethods as $key => $label)
                                <option value="{{ $key }}" {{ $transaction['method'] == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <td class="border border-gray-300 dark:border-gray-600 p-0 bg-white dark:bg-gray-900">
                        <select wire:change="updateExistingRow({{ $transaction['id'] }}, 'status', $event.target.value)"
                    ...
                    <td class="border border-gray-300 dark:border-gray-600 p-0 bg-white dark:bg-gray-900">
                        <input type="text" 
                               ...
                               data-row="existing-{{ $transaction['id'] }}" data-col="10"
                               class="w-full h-full border-0 bg-transparent dark:text-white text-sm p-3 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:bg-white dark:focus:bg-gray-800 transition-colors duration-200">
                    </td>
                    <td class="border border-gray-300 dark:border-gray-600 p-2 bg-white dark:bg-gray-900 text-center">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('focusout', function(e) {
        setTimeout(() => {
            if (!document.activeElement || !document.activeElement.hasAttribute('data-row')) {
                document.querySelectorAll('.fi-ta-cell-focused').forEach(cell => {
                    cell.classList.remove('fi-ta-cell-focused');
                });
                currentCell = null;
