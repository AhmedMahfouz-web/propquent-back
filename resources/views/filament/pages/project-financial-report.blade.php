                    ...
                    @if ($projectData)
                        @foreach ($metrics as $key => $label)
                            <tr class="{{ $loop->first ? 'border-t-2 border-gray-300 dark:border-gray-600' : '' }} bg-white dark:bg-gray-800">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right font-bold">${{ number_format($projectData['totals'][$key], 2) }}</td>
                                @foreach ($allMonths as $month)
                                    <td class="px-6 py-4 whitespace-nowrap text-right">${{ number_format($projectData['months'][$month][$key] ?? 0, 2) }}</td>
                                @endforeach
                            </tr>
                        @endforeach
                    @endif
