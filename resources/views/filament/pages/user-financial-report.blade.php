<x-filament-panels::page>
    @php
        ...
        $query = \App\Models\User::query()
            ...
            ->selectRaw(
                'SUM(CASE WHEN ut.transaction_type = "deposit" THEN ut.amount ELSE -ut.amount END) as total_net_deposit',
            );
        ...
        if ($monthsToShow->isNotEmpty()) {
            foreach ($monthsToShow as $month) {
                ...
                $query->selectRaw(
                    "SUM(CASE WHEN ut.transaction_type = \"deposit\" AND DATE_FORMAT(ut.transaction_date, '%Y-%m-01') = ? THEN ut.amount ELSE 0 END) as `Deposit_{$monthName}`",
                    [$month],
                );
                $query->selectRaw(
                    "SUM(CASE WHEN ut.transaction_type = \"withdrawal\" AND DATE_FORMAT(ut.transaction_date, '%Y-%m-01') = ? THEN ut.amount ELSE 0 END) as `Withdraw_{$monthName}`",
                    [$month],
                );
            }
            $results = $query
    ...
    <form action="{{ route('filament.admin.pages.user-financial-report') }}" method="GET"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            ...
            {{-- User Search --}}
            <div>
                <label for="search" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Search
                    User</label>
    ...
    <div class="overflow-x-auto bg-white rounded-lg shadow-sm dark:bg-gray-800">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    <th rowspan="2"
                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300 align-bottom">
                        Full Name</th>
                    <th rowspan="2"
                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300 align-bottom border-r border-gray-300 dark:border-gray-600">
                        Overall Net Deposit</th>
                    @foreach ($monthsToShow as $month)
                        <th colspan="2"
                            class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300 border-l border-gray-300 dark:border-gray-600">
                ...
                <tr>
                    @foreach ($monthsToShow as $month)
                        <th
                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300 border-l border-gray-300 dark:border-gray-600">
                            Deposit</th>
                        <th
                ...
                @forelse($results as $user)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                            {{ $user->full_name }}</td>
                        <td
                            class="px-6 py-4 whitespace-nowrap text-sm @if ($user->total_net_deposit >= 0) text-success-600 @else text-danger-600 @endif border-r border-gray-300 dark:border-gray-600">
                            {{ \Illuminate\Support\Number::currency($user->total_net_deposit, 'USD') }}</td>
                        @foreach ($monthsToShow as $month)
                            @php
                            ...
                            @endphp
                            <td
                                class="px-6 py-4 whitespace-nowrap text-sm text-success-600 border-l border-gray-300 dark:border-gray-600">
                                {{ \Illuminate\Support\Number::currency($deposit, 'USD') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-danger-600">
                                {{ \Illuminate\Support\Number::currency($withdraw, 'USD') }}</td>
                        @endforeach
                    </tr>
                @empty
                    <tr>
                        <td colspan="{{ 2 + $monthsToShow->count() * 2 }}"
                            class="px-6 py-4 text-center text-sm text-gray-500">No matching records found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
